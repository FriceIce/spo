...
const useFetchHome = () => {
  ...
  const feature = queryClient.getQueryData(["Featured playlists"]) as SpotifyApi.PlaylistObjectSimplified[]
  const newFeat = queryClient.getQueryData(["New releases"]) as SpotifyApi.AlbumObjectSimplified[]
  const recom = queryClient.getQueryData(["Recommendations"]) as unknown as SpotifyApi.TrackObjectSimplified[];  
    ...
    queries: [
      {
        ...
        queryFn: async () => 
          new Promise((resolve) => {
            ...
            spotifyApi.getMyRecentlyPlayedTracks({after: ms}).then((tracks) => {
      ...
      },
      {
        ...
        queryFn: async () => 
          ...
          .then((artistData) => {
              ...
              const seed_genres = genres.flat().filter((genre, index, self) => self.indexOf(genre) === index); 
              const formattedMusicGenres = seed_genres.slice(0, 5).join(',').replace(/(\b\s\b)/g, '-');
